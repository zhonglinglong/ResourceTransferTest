...
@log
def query_operation_record(contract_num):
    """
    查询合同操作记录
    :param contract_num:
    :return:
    ...
    try:
        ...
        return ("报错信息：" + str(e) + "sql:" + sql)

    url = "http://erp.ishangzu.com/isz_housecontract//houseContractController/selectFollowContract"
    data = {"follow_type":"","follow_content":"","create_user_name":"","contract_id":contract_id,"pageNumber":1,"pageSize":30}
    result = myRequest(url,str(data),Value=True)["obj"]["rows"][0]

    return "操作类型："+ result["follow_type_cn"] + " 内容：" + result["follow_content"] + " 操作时间："+ result["create_time"] + " 操作人：" + result["create_user_name"] + " 操作人岗位" + result["position_name"]
...
def update_apartment_contract_sign_uid(contract_num):
    ...
    data = {"contract_ids":contract_id,"sign_did":"42","sign_uid":"8A2152435DC1AEAA015DE3B72E396299","new_sign_user":"钟玲龙"}
